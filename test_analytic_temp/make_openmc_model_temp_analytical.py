...
# print(centroids.shape) printed (N,1,1,3)
# fill cells with slab
for cell in cells:
    # cell IDs run from 2 to 51, but we need to access 0 to 49 in centroids, offset 2
    mesh_id = cell.id - 2
    x = centroids[mesh_id][0][0][0] # the mesh_id th entry, first of 1 element, first of 1 element, first of 3 coordinate
    Tx = Sig_t0*T0*np.sqrt((q*L*phi0/P)*(q*L*phi0/P) - (lam -1)*x*x)
    cell.fill = slab
    cell.temperature = Tx
...
tally_global.scores = ['nu-fission','fission','kappa-fission']
