...
plt.xlim([-(L+2),L+2])
plt.xticks(bin_boundaries[::10])
plt.ylim([flux_min-0.005,flux_max+0.005])
plt.savefig('flux_dist_100.png')
plt.clf()
...
plt.savefig('relative_errors_100.png')
plt.clf()
plt.plot(xx,analytical_flux,'-bo')
plt.xlabel('Position [cm]')
plt.ylabel('Flux [n/cm^2-s]')
plt.title('Analytical Flux for 100 Mesh Points')
plt.savefig('analytical_100.png')