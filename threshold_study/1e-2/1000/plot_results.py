...
sp = openmc.StatePoint('statepoint.197.h5')
...
plt.xlim([-(L+2),L+2])
plt.xticks(bin_boundaries[::100])
plt.ylim([flux_min-0.005,flux_max+0.005])
plt.savefig('flux_dist_1000.png')
plt.clf()
...
plt.hist(flux_relative_error[nonzero], bins=1000)
...
plt.savefig('relative_errors_1000.png')
plt.clf()
plt.plot(xx,analytical_flux,'-bo')
plt.xlabel('Position [cm]')
plt.ylabel('Flux [n/cm^2-s]')
plt.title('Analytical Flux for 1000 Mesh Points')
plt.savefig('analytical_1000.png')