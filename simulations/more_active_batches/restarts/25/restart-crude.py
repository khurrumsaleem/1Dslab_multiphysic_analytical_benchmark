...
# ghost mesh with 45 regions so that 50 temperatures total are set
ghost_mesh = openmc.RegularMesh()
ghost_mesh.dimension = (N_ghost,1,1)
ghost_mesh.lower_left = (L,1,1)
ghost_mesh.upper_right = (2*L,2,2)

ghost_cells, g_cells = ghost_mesh.build_cells(['vacuum']*6)

# fill cells with slab
temp_from_csv = pd.read_csv("openmc_25_temp.csv").loc[:,"temp"]
for cell in cells:
...
root_universe = openmc.Universe(name='root universe', cells=[root_cell,ghost_cells])
